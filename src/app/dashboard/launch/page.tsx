...
export default async function LaunchPage() {
  ...
  const val = (key: string) => String(settings.launch[key] ?? "");
  return <AdminShell active="launch" title="Launch checklist" description="Every required item remains visible until it is completed and formally approved.">
    <ProgressBar percent={progress.percent} label={`${progress.completed} of ${progress.total} launch sections complete`} />
    ...
    <form action={saveStoreSettings} className="mt-5"><input type="hidden" name="scope" value="launch" /><input type="hidden" name="return_to" value="/dashboard/launch" /><SectionCard title="Formal approval record" description="Complete these only after review. The approver and reference create a clear company record."><div className="space-y-3"><Approval name="legal_approved" checked={settings.launch.legal_approved === true}>The client approved privacy, terms, refund, cancellation, shipping and data-deletion policies.</Approval><Approval name="staging_approved" checked={settings.launch.staging_approved === true}>The client approved the staging website, branding, content and product prices.</Approval><Approval name="payment_tested" checked={settings.launch.payment_tested === true}>A real low-value payment, PDF download and optional WhatsApp delivery were tested successfully.</Approval><div className="grid gap-4 pt-3 sm:grid-cols-2"><Field label="Approved by"><input name="approved_by" defaultValue={val("approved_by")} className={fieldClass} placeholder="Name and designation" /></Field><Field label="Approval reference"><input name="approval_reference" defaultValue={val("approval_reference")} className={fieldClass} placeholder="Email date or document reference" /></Field></div><div className="pt-2"><SaveButton>Save approval status</SaveButton></div></div></SectionCard></form>
  </AdminShell>;
}

function Approval({ name, checked, children }: { name: string; checked: boolean; children: React.ReactNode }) { return <label className="flex gap-3 rounded-xl border border-brand-100 p-4"><input type="checkbox" name={name} defaultChecked={checked} className="mt-0.5 h-5 w-5 shrink-0 accent-brand-teal" /><span className="text-sm text-brand-700">{children}</span></label>; }
