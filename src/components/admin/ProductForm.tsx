  ...
  return <form action={saveProduct} className="space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_340px]">
      ...
      <aside className="space-y-5">
        <section className="space-y-4 rounded-2xl border border-brand-100 bg-white p-5 shadow-[var(--shadow-card)]"><h2 className="font-bold text-brand-900">Files</h2><div>{coverPreview && <Image src={coverPreview} alt="Cover preview" width={120} height={160} className="mb-3 h-36 w-auto rounded-lg border border-brand-100 object-cover" unoptimized />}<UploadField name="cover_file" accept="image/jpeg,image/png,image/webp" label={coverPreview ? "Replace cover" : "Upload cover image"} selectedName={coverName} onFile={(file) => { setCoverPreview(URL.createObjectURL(file)); setCoverName(file.name); }} /></div>{!isCombo && <div>{product?.pdf_path && <p className="mb-2 inline-flex items-center gap-1 text-xs font-semibold text-green-600"><FileText className="h-3.5 w-3.5" /> PDF uploaded</p>}<UploadField name="pdf_file" accept="application/pdf" label={product?.pdf_path ? "Replace PDF" : "Upload customer PDF"} selectedName={pdfName} onFile={(file) => setPdfName(file.name)} /><p className="mt-2 text-[11px] text-brand-400">Stored privately and delivered only after verified payment.</p></div>}</section>
...
function UploadField({ name, accept, label, selectedName, onFile }: { name: string; accept: string; label: string; selectedName?: string; onFile?: (file: File) => void }) { return <label className="block cursor-pointer rounded-xl border border-dashed border-brand-300 px-3 py-3 text-sm font-semibold text-brand-600 hover:bg-brand-50"><span className="flex items-center gap-2"><Upload className="h-4 w-4" />{label}</span>{selectedName && <span className="mt-1 block truncate text-[11px] font-semibold text-green-600">Selected: {selectedName}</span>}<input name={name} type="file" accept={accept} className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) onFile?.(file); }} /></label>; }
