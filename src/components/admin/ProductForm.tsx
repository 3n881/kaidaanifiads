...
function UploadField({ name, accept, label, onFile }: { name: string; accept: string; label: string; onFile?: (file: File) => void }) { return <label className="flex cursor-pointer items-center gap-2 rounded-xl border border-dashed border-brand-300 px-3 py-3 text-sm font-semibold text-brand-600 hover:bg-brand-50"><Upload className="h-4 w-4" />{label}<input name={name} type="file" accept={accept} className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) onFile?.(file); }} /></label>; }
